...
var (
	...
	scriptLibrary = []Script{
		...
		{ID: "10", Name: "Whitelist Domain (Global)", Description: "Removes domain from local blocklist (Hosts)", Content: "$domain='example.com'; (Get-Content $env:windir\\System32\\drivers\\etc\\hosts) | Where-Object {$_ -notmatch $domain} | Set-Content $env:windir\\System32\\drivers\\etc\\hosts; Write-Host \"Unblocked $domain\""},
		{ID: "11", Name: "Block Domain (Global)", Description: "Adds domain to local blocklist (Hosts)", Content: "$domain='bad-site.com'; Add-Content -Path $env:windir\\System32\\drivers\\etc\\hosts -Value \"127.0.0.1 $domain\"; Write-Host \"Blocked $domain\""},
