...
## Project Structure
- **[OnPremX-Admin](file:///OnPremX-Admin)**: Central management server and dashboard.
- **[OnPremX-Agent](file:///OnPremX-Agent)**: Native Go agent for Windows endpoints (supports Service mode).
...
3. If using `.bat`: Right-click and **Run as Administrator** to automatically download, install as `OnPremXAgent` service, and start monitoring.

## Tech Stack
- **Backend**: Go (Golang), Gin Framework, GORM, SQLite (`onpremx.db`).
...
- **Agent Offline?**: Check if the `OnPremXAgent` service is running on the client machine (`services.msc`).
