...
## Project Structure
- **[System-Powershell-Admin](file:///Triveni-Powershell-Admin)**: Centralized Go Backend + React Frontend (Vite/Tailwind).
- **[System-Agent](file:///Triveni-Agent)**: Native Go agent for Windows endpoints (supports Service mode).
...
3. If using `.bat`: Right-click and **Run as Administrator** to automatically download, install as `SystemAgent` service, and start monitoring.

## Tech Stack
- **Backend**: Go (Golang), Gin Framework, GORM, SQLite (`system.db`).
...
- **Agent Offline?**: Check if the `SystemAgent` service is running on the client machine (`services.msc`).
